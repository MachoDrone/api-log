...
# Print log header before dashed section
{
...
if [ -z "$api_event_logs" ]; then
  if [ "${REQUIRE_API_OFFLINE:-0}" -eq 1 ]; then
    echo "No 'API proxy is offline, restarting..' or 'Node API is detected offline' events found in the last 24 hours." >> "$logfile"
