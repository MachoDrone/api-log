#!/bin/bash
echo "" > api-logs.txt && echo "docker log snippets:" >> api-logs.txt && podman logs -t --since 24h nosana-node 2>/dev/null | grep -C 21 "API proxy is offline, restarting.." >> api-logs.txt && echo "" >> api-logs.txt
...
echo "" >> api-logs.txt && podman logs -t nosana-node | grep 'Wallet:' | awk '{print "Host Address: " $3}' >> api-logs.txt
